<?php 
 ...
 $exams = $_GET['exams'];
        ...
        if(isset($_POST['submit'])) {  
            ...
            $query = "SELECT student_name FROM gh WHERE student_name = '$stdName' ";
            $result = mysqli_query($connect,$query);
            if(mysqli_num_rows($result)> 0) {
                if(!empty($_POST['indTest'])) {
                    $indTest = $_POST['indTest'];
                }else {
                    $query= "SELECT individual_test FROM gh WHERE student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                ...
                if(!empty($_POST['classTest'])) {
                    ...
                    $query= "SELECT class_test FROM gh WHERE student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                ...
                if(!empty($_POST['groupWork'])) {
                    ...
                    $query= "SELECT group_work FROM gh WHERE student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                ...
                if(!empty($_POST['project'])) {
                    ...
                    $query= "SELECT project FROM gh WHERE student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                        $project = $row['project'];
                    }
                ...
                if(!empty($_POST['exams'])) {
                    ...
                    $query= "SELECT exams FROM gh WHERE student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                        $exams = $row['exams'];
                    }
                ...
                if(!(empty($indTest)&empty($classTest)&empty($groupWork)&empty($project)&empty($exams))) {
                    ...
                    if($result) {
                        ...
                        return header('location:./ghp3.php');
                    }
                }
                else {
                    $query = "UPDATE
                ...
                // echo "<h4 style='color:red;text-align:center'>Add student before you can edit";
                return header('location:./ghp3.php');
            }
