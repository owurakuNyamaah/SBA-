<?php 
 ...
 $exams = $_GET['exams'];
        ...
        if(isset($_POST['submit'])) {  
            ...
            $query = "SELECT student_name from arts where student_name='$stdName'";
            $result = mysqli_query($connect,$query);
            if(mysqli_num_rows($resutl)>0) {
                if(!empty($_POST['indTest'])) {
                    $indTest = $_POST['indTest'];
                }else {
                    $query= "SELECT individual_test from arts where student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                ...
                if(!empty($_POST['classTest'])) {
                    ...
                    $query= "SELECT class_test from arts where student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                ...
                if(!empty($_POST['groupWork'])) {
                    ...
                    $query= "SELECT group_work from arts where student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                ...
                if(!empty($_POST['project'])) {
                    ...
                    $query= "SELECT project from arts where student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                        $project = $row['project'];
                    }
                ...
                if(!empty($_POST['exams'])) {
                    ...
                    $query= "SELECT exams from arts where student_name='$stdName'";
                    $result = mysqli_query($connect,$query);
                    while($row=mysqli_fetch_assoc($result)) {
                        $exams = $row['exams'];
                    }
                ...
                if(!(empty($indTest)&empty($classTest)&empty($groupWork)&empty($project)&empty($exams))) {
                    ...
                    if($result) {
                        ...
                        return header('location:./creative4.php');
                    }
                }
                else {
                    $query = "UPDATE
