...
<body>
    <header>
        ...
        <form action = './rme2.php'><button class = 'std'>Return to SBA</button></form>
    </header>

    <form method = 'post' action = 'rmeEdit2.php' class = 'container'>
    ...
    <?php 
        ...
        if(isset($_POST['submit'])) {  
            ...
            if(!empty($_POST['indTest'])) {
                ...
                $query= "SELECT individual_test FROM rme WHERE student_name='$stdName'";
            ...
            if(!empty($_POST['classTest'])) {
                ...
                $query= "SELECT class_test FROM rme WHERE student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
            ...
            if(!empty($_POST['groupWork'])) {
                ...
                $query= "SELECT group_work FROM rme WHERE student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
            ...
            if(!empty($_POST['project'])) {
                ...
                $query= "SELECT project FROM rme WHERE student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
                    $project = $row['project'];
                }
            ...
            if(!empty($_POST['exams'])) {
                ...
                $query= "SELECT exams FROM rme WHERE student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
                    $exams = $row['exams'];
                }
