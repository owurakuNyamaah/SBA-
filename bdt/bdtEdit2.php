...
<body>
    <header>
        ...
        <form action = './bdt2.php'><button class = 'std'>Return to SBA</button></form>
    </header>

    <form method = 'post' action = 'bdtEdit2.php' class = 'container'>
    ...
    <?php 
        ...
        if(isset($_POST['submit'])) {  
            ...
            if(!empty($_POST['indTest'])) {
                ...
                $query= "SELECT individual_test from bdt where student_name='$stdName'";
            ...
            if(!empty($_POST['classTest'])) {
                ...
                $query= "SELECT class_test from bdt where student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
            ...
            if(!empty($_POST['groupWork'])) {
                ...
                $query= "SELECT group_work from bdt where student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
            ...
            if(!empty($_POST['project'])) {
                ...
                $query= "SELECT project from bdt where student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
                    $project = $row['project'];
                }
            ...
            if(!empty($_POST['exams'])) {
                ...
                $query= "SELECT exams from bdt where student_name='$stdName'";
                $result = mysqli_query($connect,$query);
                while($row=mysqli_fetch_assoc($result)) {
                    $exams = $row['exams'];
                }
